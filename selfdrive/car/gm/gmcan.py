...
def create_gm_acc_spam_command(packer, controller, CS, slcSet, bus, Vego, frogpilot_toggles, accel, experimentalMode, sdgm):
  cruiseBtn = CruiseButtons.INIT
  byfive = 0
  ...
  if slcSet <= int(math.floor((speedSetPoint - 1)/5.0)*5.0) and speedSetPoint > (25 if frogpilot_toggles.is_metric else 20) and sdgm:
    cruiseBtn = CruiseButtons.DECEL_SET
    byfive = 1
  elif slcSet >= int(math.ceil((speedSetPoint + 1)/5.0)*5.0) and sdgm:
    cruiseBtn = CruiseButtons.RES_ACCEL
    byfive = 1
  elif slcSet < speedSetPoint and speedSetPoint > (25 if frogpilot_toggles.is_metric else 16):
    cruiseBtn = CruiseButtons.DECEL_SET
    byfive = 0
